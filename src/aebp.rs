...
pub fn main() {
    ...
    let mut pi_sums = inverse_pseudo_euler_transform_mod(id_mod);
    dbg!(start.elapsed());

    let mut unit = FIArrayU32::new(N);
    for (i, v) in FIArrayU32::keys(N).enumerate() {
        unit.arr[i] = (v as u64 % MOD) as u32;
    }
    let pi = inverse_pseudo_euler_transform_mod(unit);
    dbg!(start.elapsed());
    ...
    for i in 0..pi_sums.arr.len() {
        let (p, u) = (pi_sums.arr[i] as u64, pi.arr[i] as u64);
        pi_sums.arr[i] = ((A * u + B * p) % MOD) as u32;
    }
    dbg!(start.elapsed());

    let approx = pseudo_euler_transform_mod(pi_sums);
    dbg!(start.elapsed());

    let accurate = mult_correction(&approx, &primes, |_, p, e| {
        let (p, e) = (p as u64, e as u64);
        ((A * e + B * p) % MOD) as u32
    });
    dbg!(start.elapsed());
...
fn pseudo_euler_transform_mod(a: FIArrayU32) -> FIArrayU32 {
    ...
    for i in x..=len {
        ret.arr[i - 1] += ((u64::from(v_3.arr[i - 1]) * Reciprocal[6]) % MOD) as u32;
        if ret.arr[i - 1] >= MOD as u32 {
            ret.arr[i - 1] -= MOD as u32;
        }
    }
    let v_4 = mult_sparse_mod(&v, &v_3);
    for i in x..=len {
        ret.arr[i - 1] += ((u64::from(v_4.arr[i - 1]) * Reciprocal[24]) % MOD) as u32;
        if ret.arr[i - 1] >= MOD as u32 {
            ret.arr[i - 1] -= MOD as u32;
        }
    }
...
fn inverse_pseudo_euler_transform_mod(a: FIArrayU32) -> FIArrayU32 {
    ...
    let a_3 = mult_mod(&a, &a_2);

    for i in x..=len {
        ret.arr[i - 1] += ((u64::from(a_3.arr[i - 1]) * Reciprocal[3]) % MOD) as u32;
        if ret.arr[i - 1] >= MOD as u32 {
            ret.arr[i - 1] -= MOD as u32;
        }
    }
    let a_4 = mult_sparse_mod(&a, &a_3);
    for i in x..=len {
        ret.arr[i - 1] += MOD as u32 - ((u64::from(a_4.arr[i - 1]) * Reciprocal[4]) % MOD) as u32;
        if ret.arr[i - 1] >= MOD as u32 {
            ret.arr[i - 1] -= MOD as u32;
        }
    }
