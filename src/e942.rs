...
pub fn main() {
    ...
    let step = (small[B - 1] << 1) % MOD; // 2^B mod MOD
    ...
    let p_mod = (pow2_at(Q) + MOD - 1) % MOD;

    // 2 is an order q root of unity modulo p, therefore
    // S = sum_{a=1}^{q-1} (a/q)2^a = 2 * sum_{t=1}^{(q-1)/2} 2^{t^2 mod q} - (2^q - 2)
    ...
    // S_mod = (2*sum_res + 2 - 2^q) mod MOD
    let two_sum = (sum_res << 1) % MOD;
    let mut s_mod = (two_sum + 1) % MOD;
    if s_mod >= p_mod {
        s_mod -= p_mod;
    } else {
        s_mod = (s_mod + MOD) - p_mod;
    }

    // Decide minimal root: check χ(q-2).
    let r_mod = if CHI_Q_MINUS_2 == 1 {
        // S >= p/2
        // minimal is p - S
        if p_mod >= s_mod {
            p_mod - s_mod
